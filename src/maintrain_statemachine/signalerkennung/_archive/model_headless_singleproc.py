...
if __name__ == '__main__':
    ...
    for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
        ...
        for i in range(0, len(contours)):
            ...
            #number = cv2.threshold(gray[top_border:bottom_border, left_border:right_border], 30, 255, cv2.THRESH_BINARY)[1]

            #Number for model (better than the binary one)
            number_model_base = gray[top_border:bottom_border, left_border:right_border]

            # calculate right and bottom row of array
            bottom_row, right_col = number_model_base.shape
            bottom_row -= 1
            right_col -= 1

            # invert if signal is black
            if number_model_base[bottom_row, :].all() == 0:
                number_model_base = cv2.bitwise_not(number_model_base)

            # calculate relation of black and white
            nonzeroes = np.count_nonzero(number_model_base) / np.prod(number_model_base.shape)

            # check if bottom row and right col complete white
            if number_model_base[:, right_col].all() != 0 and number_model_base[bottom_row, :].all() != 0 and nonzeroes > 0.6 and nonzeroes < 0.9:
                # Template
                #get_number_temp_match, res = compare_template(number)
                #get_number_temp_match = tesseract(number)
                get_number_model = detect(number_model_base)


                try:
                    number_check = int(get_number_model) #check for no number
                    print("[Recognized][Proc-%s]: Got some Number %s"%(counter,str(get_number_model)))
